...
const AuditForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Формируем сообщение для Telegram с пометкой "Аудит"
    ...
    return (
      <div className="bg-green-50 border border-green-200 rounded-lg p-6 text-center">
        <div className="text-green-600 text-lg font-medium mb-2">
          ✅ Запрос на аудит отправлен!
        </div>
        <p className="text-green-700 mb-4">
          Мы свяжемся с вами для проведения аудита ваших процессов
        </p>
        <Button 
          onClick={() => {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label htmlFor="message" className="block text-sm font-medium text-gray-700 mb-1">
          Опишите ваши процессы (необязательно)
        </label>
        <textarea
          id="message"
          name="message"
          ...
          placeholder="Расскажите о процессах, которые хотите автоматизировать..."
        />
      </div>
      ...
      >
        {isSubmitting ? 'Отправляем...' : 'Запросить аудит'}
      </Button>
    </form>
  );
};
