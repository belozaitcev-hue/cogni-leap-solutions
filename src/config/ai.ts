      ...
      const response = await fetch(AI_CONFIG.groq.apiUrl, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'Ты Дмитрий - AI-помощник, который помогает с автоматизацией бизнеса с помощью нейросетей. Отвечай на русском языке, будь дружелюбным и профессиональным. Если не знаешь ответа, предложи связаться с экспертами.'
            },
            {
              role: 'user',
              content: message
      ...
      const response = await fetch(AI_CONFIG.openai.apiUrl, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'Ты Дмитрий - AI-помощник, который помогает с автоматизацией бизнеса с помощью нейросетей. Отвечай на русском языке, будь дружелюбным и профессиональным.'
            },
            {
              role: 'user',
              content: message
...
const getFallbackResponse = (message: string): string => {
  ...
  if (lowerMessage.includes('привет') || lowerMessage.includes('здравствуйте') || lowerMessage.includes('добрый')) {
    return 'Привет! Я Дмитрий, AI-помощник. Расскажу о наших услугах по автоматизации бизнеса с помощью нейросетей. Что вас интересует?';
  }
